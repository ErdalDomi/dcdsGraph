...
//upgrade with this https://stackoverflow.com/questions/45329461/query-on-is-not-a-function
...
app.post("/loadFrontier", function(req, res) {
    if (sessionDBtype == 'psql') {
        var nodeArray = req.body.currentNodeID.split(',');
        var queryString = 'select * from "TS" where ';

        for (var i = 0, len = req.body.currentNodeID.split(',').length; i < len; i++) {
            queryString += 'curr = ' + nodeArray[i] + ' or ';
        }
        queryString = queryString.substring(0, queryString.length - 4) + ';';
        console.log("Query string is: " + queryString);
        var query = client.query(queryString);
        query.on('error', function(error) {
            console.log("Can't expand anymore. " + error + ". Query: " + queryString);
